...
// Serializes permission requests: parallel sub-agents may ask at the same
// time, but the user only answers one at a time. The lock also wraps rule
// checking (in the confirm* functions): if the first request saves an "always
// allow", the equivalent request that was waiting is auto-approved on wakeup.
let askChain: Promise<void> = Promise.resolve();

function withAskLock<T>(fn: () => Promise<T>): Promise<T> {
  const result = askChain.then(fn);
  askChain = result.then(
    () => undefined,
    () => undefined,
  );
  return result;
...
async function promptChoice(prompt: string, choices: string[]): Promise<string> {
  ...
  try {
    for (;;) {
      const raw = (await rl.question(`${prompt} [${choices.join("/")}] (n): `))
        .trim()
        .toLowerCase();
      if (raw === "") return "n";
...
async function ask(
  ...
  printOut(`\n${style("1;33", "⚠ Permission:")} ${action}`);
  if (preview) printOut(style("2", preview));
  if (banned) {
    const choice = await promptChoice("[y]es once / [n]o", ["y", "n"]);
    return choice === "y";
  }
  const choice = await promptChoice(
    `[y]es once / [a]lways allow ${style("36", alwaysPattern)} / [n]o`,
    ["y", "a", "n"],
  );
  if (choice === "a") {
    saveRule(kind, alwaysPattern);
    printOut(style("2", `Rule saved: ${kind} → ${alwaysPattern}`));
    return true;
  }
  return choice === "y";
}
