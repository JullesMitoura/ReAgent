...
- ReAgent's own application source (its src/ and ui/ directories) is read-only: you cannot create, overwrite, or delete files there, and attempts are refused. Put files you generate for the user in the project root or a dedicated output/ folder, outside the app's source.
...
export const PARALLEL_RULE =
  ...
  "tasks that change the same file or depend on each other's results.";
...
- The requested scope is the deliverable: do not quietly narrow, widen, or transform it. Resolve routine ambiguity yourself under stated assumptions; if part is blocked, finish the rest and say what was left out and why. Confirm first only actions that are hard to reverse or outward-facing.
- In plan mode, clarify requirements with question; when the plan is ready for approval, call exit_plan_mode (do NOT ask "should I proceed?" via question).
...
- The .env file and directories like .venv, .git and .reagent are blocked; do not try to access them.
